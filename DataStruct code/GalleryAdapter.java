...
/*
Adapter that will handle the RecyclerView data and display in grid format
 */
...
public class GalleryAdapter extends RecyclerView.Adapter<GalleryAdapter.GalleryViewHolder> {
    ...
    //Using click listener to handle click events
    ...
    //onBindViewHolder method is where we bind the data to each layout item based on position
    @Override
    public void onBindViewHolder(final GalleryViewHolder holder, int position) {
        final ImageModel imageModel = galleryList.get(position);

        //Using Glide to load images to ImageView
        //Glide loads using lower resolution then transitions to a higher quality one
        Glide.with(holder.galleryImageView.getContext()).
                load(imageModel.getImage_ID())
                .thumbnail(0.5f)
